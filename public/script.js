function drawChord(peerDataFromBackend) {
  ...
  function makeColorsForIds(data) {
    ...
    getUniqIds(data).forEach(uniqId => {
      colors.push(globalColors[uniqId].color) //add for each id a generated color and return a new array
    ...
    getUniqIds(data)
      ...
      .forEach((uniqId, uniqIdIndex) => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (uniqIdIndex == 0 && i == 0) {
            // we need this, so the value for the element and itself is always 0.
            matrixEntry.push(0)
          }
          ...
          if (nextIndexForMatrix === uniqIdIndex) {
            // we need this, so the value for the element and itself is always 0.
            matrixEntry.push(0)
          }
        }
  ...
  function mouseoverChord(d, i) {
    ...
    $(this).popover({
      ...
      content: function() {
        return (
          "<p style='font-size: 11px; text-align: center;'>the average roundtrip between <span style='font-weight:900'>" +
          names[d.source.index] +
          "</span> and <span style='font-weight:900'>" +
          names[d.target.index] +
          "</span> is currently <span style='font-weight:900'>" +
          d.source.value +
          '</span>ms</p>'
        )
      }
  ...
  function reDraw() {
    ...
    d3.select('#chord')
      .select('svg')
      .remove() //remove old diagram
    let svg = d3
      .select('#chord')
      .append('svg')
      .attr('height', height)
      .attr('width', width) //draw new diagram
