;(function() {
  ...
  function buildFoci() {
    let peerDataFromBackend = JSON.parse(
    ...
    let width = 800,
      height = 800,
      angle = 0,
      step = (2 * Math.PI) / amountOfClusters

    uniqIdsOfPeers.forEach((peerId, peerIndex) => {
      let x = Math.round(width / 2 + radius * Math.cos(angle) - width / 2) + 450
      let y =
        Math.round(height / 2 + radius * Math.sin(angle) - height / 2) + 450

      foci[peerId] = {
        x: x,
        y: y,
        color: globalColors[peerId].color,
        slow: false
      }
  ...
  function timer() {
    ...
    let thisConnectionIsSlow = determineSlowConnection(
      choice,
      oldChoice ? oldChoice : ''
    )
  ...
  function gravity(alpha, eAlpha) {
    return function(d) {
      d.y +=
        (foci[d.choice].y - d.y) * (d.slow ? 0.007 * eAlpha : alpha * eAlpha)
      d.x +=
        (foci[d.choice].x - d.x) * (d.slow ? 0.007 * eAlpha : alpha * eAlpha)
    }
  }
